...
module.exports = () => new Promise((accept, reject) => {
  wpwithin.createClient(host, port, true, logFileName, function (error, client) {

    if (error) {
      reject(reject);
    }

    accept(() => new Promise((accept, reject) => {
      setup()
      ...
      function discoverDevices() {
        client.searchForDevice(10000, "Trackeroo", (error, res) => {
          if (error) {
            reject(new Error(error));
            return;
          }
